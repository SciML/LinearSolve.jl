...
init_cacheval(
    alg::Union{LUFactorization,GenericLUFactorization},
    A,
    b,
    u,
    Pl,
...
function init_cacheval(
    ...
    res = SuiteSparse.UMFPACK.UmfpackLU(
        C_NULL,
        C_NULL,
        size(A, 1),
        size(A, 2),
...
init_cacheval(
    alg::GenericFactorization{typeof(lu)},
    A::StridedMatrix{<:LinearAlgebra.BlasFloat},
    b,
    u,
    Pl,
    Pr,
...
init_cacheval(
    ...
    reltol,
    verbose,
) where {T,V} = LinearAlgebra.LDLt{T,SymTridiagonal{T,V}}(A)
...
function init_cacheval(
    alg::Union{QRFactorization,SVDFactorization},
    A::StridedMatrix{<:LinearAlgebra.BlasFloat},
    b,
    u,
    Pl,
...
RFLUFactorization(; pivot = Val(true), thread = Val(true)) =
    GenericFactorization(; fact_alg = RFWrapper(pivot, thread))

init_cacheval(
    alg::GenericFactorization{<:RFWrapper},
    A,
    b,
    u,
    Pl,
