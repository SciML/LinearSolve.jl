...
function KrylovJL(
    ...
    return KrylovJL(KrylovAlg, gmres_restart, window, args, kwargs)
end
...
function SciMLBase.solve(cache::LinearCache, alg::KrylovJL; kwargs...)
    ...
    kwargs = (atol = atol, rtol = rtol, itmax = itmax, verbose = verbose, alg.kwargs...)

    if cache.cacheval isa Krylov.CgSolver
        N !== I && @warn "$(alg.KrylovAlg) doesn't support right preconditioning."
        Krylov.solve!(args...; M = M, kwargs...)
    elseif cache.cacheval isa Krylov.GmresSolver
        Krylov.solve!(args...; M = M, N = N, kwargs...)
...
function IterativeSolversJL(
    ...
    return IterativeSolversJL(generate_iterator, gmres_restart, args, kwargs)
end

IterativeSolversJL_CG(args...; kwargs...) = IterativeSolversJL(
    args...;
    generate_iterator = IterativeSolvers.cg_iterator!,
    kwargs...,
)
IterativeSolversJL_GMRES(args...; kwargs...) = IterativeSolversJL(
    args...;
    generate_iterator = IterativeSolvers.gmres_iterable!,
    kwargs...,
)
IterativeSolversJL_BICGSTAB(args...; kwargs...) = IterativeSolversJL(
    args...;
    generate_iterator = IterativeSolvers.bicgstabl_iterator!,
    kwargs...,
)
IterativeSolversJL_MINRES(args...; kwargs...) = IterativeSolversJL(
    args...;
...
function init_cacheval(
    ...
    kwargs = (abstol = abstol, reltol = reltol, maxiter = maxiters, alg.kwargs...)
    ...
    iterable = if alg.generate_iterator === IterativeSolvers.cg_iterator!
        ...
        alg.generate_iterator(u, A, b, Pl; kwargs...)
    elseif alg.generate_iterator === IterativeSolvers.gmres_iterable!
        alg.generate_iterator(u, A, b; Pl = Pl, Pr = Pr, restart = restart, kwargs...)
...
function KrylovKitJL(args...; KrylovAlg = KrylovKit.GMRES, gmres_restart = 0, kwargs...)
