...
@testset "Sparse Matrix Caching Allocation Tests" begin
    ...
    for alg in sparse_algs
        @testset "$(typeof(alg))" begin
            ...
            # First solve - this will create the factorization
            sol1 = solve!(cache)
            @test norm(test_A * sol1.u - b1) < 1.0e-10
            ...
            try
                @test_nowarn solve_no_alloc!(cache, b2)
                @test norm(test_A * cache.u - b2) < 1.0e-10

                # Test one more time with different b
                @test_nowarn solve_no_alloc!(cache, b3)
                @test norm(test_A * cache.u - b3) < 1.0e-10
            catch e
...
@testset "Iterative Solver Caching Allocation Tests" begin
    ...
    for alg in iterative_algs
        @testset "$(typeof(alg))" begin
            ...
            # First solve
            sol1 = solve!(cache)
            @test norm(A * sol1.u - b1) < 1.0e-6  # Looser tolerance for iterative methods
            ...
            try
                @test_nowarn solve_no_alloc!(cache, b2)
                @test norm(A * cache.u - b2) < 1.0e-6

                # Test one more time with different b
                @test_nowarn solve_no_alloc!(cache, b3)
                @test norm(A * cache.u - b3) < 1.0e-6
            catch e
...
@testset "Matrix Change Refactorization Tests" begin
    ...
    for alg in algs
        @testset "$(typeof(alg))" begin
            ...
            # First solve
            sol1 = solve!(cache)
            @test norm(test_A1 * sol1.u - b) < 1.0e-10
            @test !cache.isfresh
            ...
            try
                ...
                @test norm(test_A2 * cache.u - b_new) < 1.0e-10
            catch e
                @test_broken false
            end
        end
